...
#include "Misc.h"
#include "../include/libcore/ScopedPthreadMutexLock.h"
#include "../include/utils/UniquePtr.h"
...
public:
    explicit ScopedJniThreadState(JNIEnv *env) {
        ...
        if (UNLIKELY(gDvmJni.workAroundAppJniBugs)) {
            ...
            if (self != mSelf) {
                ALOGE("JNI ERROR: env->self != thread-self (%p vs. %p); auto-correcting", mSelf,
                      self);
                mSelf = self;
            }
        }
...
void dvmDumpJniStats(DebugOutputTarget *target) {
    ...
    dvmPrintDebugMessage(target, "; workarounds are %s",
                         gDvmJni.workAroundAppJniBugs ? "on" : "off");
...
static void
dumpCandidateMethods(ClassObject *clazz, const char *methodName, const char *signature) {
...
static void
throwNoSuchMethodError(ClassObject *c, const char *name, const char *sig, const char *kind) {
...
jint JNI_CreateJavaVM(JavaVM **p_vm, JNIEnv **p_env, void *vm_args) {
     ...
     */
    //创建jniEnv，其实就是分配空间，绑定线程指针（JNIEnv线程不安全，每个JNIEnv都有所属线程，其中JNIEnv内部的functionTable）
