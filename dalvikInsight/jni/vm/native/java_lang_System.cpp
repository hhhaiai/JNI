...
static void Dalvik_java_lang_System_arraycopy(const u4 *args, JValue *pResult) {
    ...
    if (srcPrim || dstPrim) {
        ...
        if (false)
            ALOGD("arraycopy prim[%c] dst=%p %d src=%p %d len=%d",
        ...
        if (srcClass->arrayDim == dstClass->arrayDim &&
             ...
             */
            if (false)
                ALOGD("arraycopy ref dst=%p %d src=%p %d len=%d",
            ...
            if (false)
                ALOGD("arraycopy iref dst=%p %d src=%p %d count=%d of %d",
                      dstArray->contents, dstPos * width,
                      srcArray->contents, srcPos * width,
                      copyCount, length);
            move32((u1 *) dstArray->contents + dstPos * width,
