...
void dvmDdmSendHeapInfo(int reason, bool shouldLock) {
    ...
    /* For each heap (of which there is one) */
    {
        ...
        *b++ = (u1) reason;
        ...
        set4BE(b, dvmHeapSourceGetMaximumSize());
        b += 4;

        /* current heap size in bytes */
        set4BE(b, dvmHeapSourceGetValue(HS_FOOTPRINT, NULL, 0));
        b += 4;

        /* number of bytes allocated */
        set4BE(b, dvmHeapSourceGetValue(HS_BYTES_ALLOCATED, NULL, 0));
        b += 4;

        /* number of objects allocated */
        set4BE(b, dvmHeapSourceGetValue(HS_OBJECTS_ALLOCATED, NULL, 0));
        b += 4;
    }
